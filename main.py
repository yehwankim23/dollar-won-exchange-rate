...
def get_exchange_rate():
    ...
    response = requests.get(
        "https://www.kebhana.com/cms/rate/wpfxd651_01i_01.do",
        params={
            "ajax": "true",
            "curCd": "USD",
            "tmpInqStrDt": now.strftime("%Y-%m-%d"),
            "pbldDvCd": "3",
            "inqStrDt": now.strftime("%Y%m%d"),
            "inqKindCd": "1",
    ...
    exchange_rate = td_list[8].text.strip()
...
def main():
    ...
    try:
        ...
        current_float += 2.5
        previous_division = current_float // 5

        send_log(f"Program started ({current_string} 원)")
    ...
    while True:
        ...
        try:
            ...
            if now.hour == 12:
                if check_running:
                    current_string, _ = get_exchange_rate()
                    send_log(f"Program running ({current_string} 원)")
